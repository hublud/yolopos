  ...
  return (
    <div className="p-3.5 sm:p-6 md:p-8 h-full flex flex-col relative bg-gray-50">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 sm:mb-6">
        <div>
          <h2 className="text-xl sm:text-2xl font-black text-yolo-dark">Inventory Management</h2>
          <p className="text-xs sm:text-sm text-gray-500">Monitor stock levels, manage drinks menu, prices and variants</p>
        </div>
        <button 
          onClick={() => {
            setNewVariants([])
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          ...
          <table className="hidden sm:table w-full text-left border-collapse">
            <thead className="bg-gray-50 sticky top-0 z-10 text-xs font-bold text-gray-500 uppercase">
              <tr>
                <th className="px-6 py-3.5">Item Info</th>
                <th className="px-6 py-3.5">Category</th>
                <th className="px-6 py-3.5">Price / Options</th>
              ...
              {loading && products.length === 0 ? (
                ...
                return (
                  <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-3.5 font-bold text-yolo-dark">
                      {variants.length > 0 ? (
          ...
          <div className="sm:hidden divide-y divide-gray-100 p-2">
            {filteredProducts.map((p: any) => {
              const variants = (p.variants || []).filter((v: any) => v.active !== false)
              return (
                <div key={p.id} className="py-3 px-2 flex flex-col gap-2">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 overflow-hidden bg-gray-50 border border-gray-100 shadow-sm">
                        <ProductImage image={p.image} category={p.category} />
                      </div>
                  ...
                  <div className="flex items-center justify-between text-xs pt-1 border-t border-gray-50">
                    ...
                    <div className="flex items-center gap-1.5">
                      <button 
                        onClick={() => openEditModal(p)}
                        className="px-2.5 py-1 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg text-[11px] font-bold flex items-center gap-1"
                      >
                        <Edit2 size={12} />
                        Edit
                      </button>
                      <button 
                        ...
                        className="px-2.5 py-1 text-green-700 bg-green-50 hover:bg-green-100 rounded-lg text-[11px] font-bold flex items-center gap-1"
                      >
                        <TrendingUp size={12} />
                        Stock
                      </button>
          ...
          {!loading && filteredProducts.length === 0 && (
            <div className="p-8 text-center text-gray-400 text-xs">
              <Package className="mx-auto mb-2 opacity-40 text-gray-400" size={36} />
              No products found.
            </div>
          )}
        </div>
